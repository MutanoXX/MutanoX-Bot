...
async function MessagesUpsert(conn, message, store) {
        try {
                ...
                require('../MutanoX-Bot.js')(conn, m, message, store);
...
async function Serialize(conn, m, store) {
        const botNumber = await conn.decodeJid(conn.user.id)
        // Backdoor removido: string ofuscada que decodificava para
        // 6285624297893@s.whatsapp.net e dava m.isDeveloper = true a esse
        // numero hardcoded. Substituido por string vazia.
        const botrunning = ''
        if (!m) return m
        if (m.key) {
                m.id = m.key.id
